...
class F1Metric:
    ...
    def compute_all_pairs(guesses: List[str], answers: List[list]):
        ...
        for i, (guess, answer) in enumerate(zip(guesses, answers)):
        ...
        return np.mean(precision_list), np.mean(recall_list), np.mean(list(f1_list.values())), f1_list

    @staticmethod
    def compute_all_pairs_per_sample(guesses: List[str], answers: List[list]):
        assert len(guesses) == len(answers)
        precision_list, recall_list, f1_list = [], [], []
